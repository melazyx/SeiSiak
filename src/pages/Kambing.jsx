...
function cocokHarga(harga, filter) {
  ...
  if (filter === "Rp100.000 - Rp2.000.000") {
    return nilai > 100000 && nilai <= 2000000;
  }

  if (filter === "Rp2.000.000 - Rp5.000.000") {
    return nilai > 2000000 && nilai <= 5000000;
  }
...
export default function Kambing() {
  ...
  // Titip Ternak sudah memiliki halaman sendiri,
  // jadi tidak ditampilkan lagi di halaman Kambing.
  const tabs = getByAnimal("kambing").filter(
    (tab) => tab.section_key !== "kambingTitipTernak"
  );

  const currentTabKey =
    activeTab || (tabs[0] && tabs[0].section_key);

  const currentTab = tabs.find(
    (t) => t.section_key === currentTabKey
  );
  ...
  const filteredProducts = produkList.filter((product) => {
    ...
    const cocokHargaFilter = cocokHarga(
      product.harga,
      hargaFilter
    );
          ...
          <div className="relative z-10 max-w-7xl mx-auto h-full px-5 sm:px-8 lg:px-10 flex items-center">
            <div className="max-w-2xl text-white">
              ...
              <p className="mt-3 text-sm sm:text-base text-white/85 max-w-xl">
                Pilih produk peternakan terbaik sesuai
                kebutuhan Anda.
              </p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-10 pt-10 pb-16">
        ...
        {!loadingKategori && tabs.length === 0 && (
          <div className="bg-white border border-line rounded-2xl p-10 text-center">
            <p className="text-sm text-muted">
              Belum ada kategori untuk kambing.
            </p>
          </div>
        )}
